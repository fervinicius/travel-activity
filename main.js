...
const atividade = {
  nome: "Lunch",
  data: new Date("2024-07-11 18:18"),
  finalizada: true
}
...
let atividades = [
  atividade,
  {
    nome: 'Afternoon Snack',
    data: new Date("2024-07-11 19:05"),
    finalizada: false
  },
  {
    nome: 'Dinner',
    data: new Date("2024-07-11 19:07"),
    finalizada: false
  },
]
...
function atualizarListaDeAtividades () {
  ...
  if(atividades.length == 0) {
    ...
    // IMPEDE DO FLUXO DA LÓGICA CONTINUAR APÓS FOR VERDADE
    return
  }

  for(let atividade of atividades) {
     //ITERAÇÃO DE VALOR (CONTADOR)
...
const salvarAtividade = (event) => {
  ...
  const novaAtividade = {
    // NOME DA PROPRIEDADE IGUAL O NOME DO OBJETO 
    nome,
    data,
    finalizada: false
  }
...
const criarHoras = () => {
  ...
  for(let i = 6; i <= 22; i++) {
    ...
    horasDisponiveis += `<option value="${hora}:00">${hora}:00h</option>`
    horasDisponiveis += `<option value="${hora}:30">${hora}:30h</option>`
  }
